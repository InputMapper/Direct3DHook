...
namespace Capture.Hook
{
    internal class DXHookD3D9 : BaseDXHook
    {
        ...
        public override void Hook()
        {
            ...
            try
            {
                using (var d3dEx = new Direct3DEx())
                {
                    ...
                    {
                        using (
                            var deviceEx = new DeviceEx(d3dEx, 0, DeviceType.NullReference, IntPtr.Zero,
                                CreateFlags.HardwareVertexProcessing,
                                new PresentParameters
                                {
                                    BackBufferWidth = 1,
        ...
        private unsafe delegate int Direct3D9Device_PresentDelegate(
            IntPtr devicePtr, Rectangle* pSourceRect, Rectangle* pDestRect, IntPtr hDestWindowOverride,
            IntPtr pDirtyRegion);
